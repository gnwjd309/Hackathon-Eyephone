...
import com.example.eyephone_project.R;
import com.example.eyephone_project.calibration.CalibrationDataStorage;
import com.example.eyephone_project.CalibrationViewer;
import com.example.eyephone_project.PointView;
                    ...
                    if (!gazeTracker.isCalibrating()) {
                        if (isUseGazeFilter) {
                            if (oneEuroFilterManager.filterValues(gazeInfo.timestamp, gazeInfo.x, gazeInfo.y)) {
                                ...
                                count=0;
                                for(int i=1;i<10;i++){
                                    if(xq.QArray[0]+80>xq.QArray[i] && xq.QArray[0]-80 < xq.QArray[i] && yq.QArray[0]+170>yq.QArray[i] && yq.QArray[0]-170 < yq.QArray[i]){
                                        count++;
                                    }
                                }
                                if(count>6&&safebar<0) {
